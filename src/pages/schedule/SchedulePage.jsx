...
const SchedulePage = () => {
  ...
  const weekSessions = viewMode === VIEW_MODES.WEEK 
    ? sessions.filter(s => weekDays.some(d => isSameDay(parseSessionDate(s.sessionDate), d)))
    : [];
  const daySessions = viewMode === VIEW_MODES.DAY ? getSessionsForDate(currentDate) : [];
    ...
    <ClassroomDetailLayout>
      <div className="classroom-schedule-page">
        ...
        {!loading && !error && viewMode === VIEW_MODES.DAY && (() => {
          const daySessionsView = getSessionsForDate(currentDate);
          const currentTime = getCurrentTimeInfo();
